...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        address from = _owner;
        _owner = newOwner;
        emit OwnershipTransferred(from, _owner);
    }
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == ownerOf(tokenId) || isApprovedForAll(_tokenOwner[tokenId], to) == true);
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        if(_exists(tokenId) == true || to == address(0)) {
            revert();
        }
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId) == true, "Token not found");
        string memory tokenAsString = usingOraclize.uint2str(tokenId);
        
        // concatinate baseurl with token Id
        _tokenURIs[tokenId] = usingOraclize.strConcat(_baseTokenURI, tokenAsString);
    }
...
contract MSDRealEstateToken is ERC721Metadata("MSDToken", "MSDR", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
